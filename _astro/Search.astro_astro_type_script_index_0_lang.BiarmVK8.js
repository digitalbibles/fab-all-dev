function E(e,s){if(e===s)return 0;const t=Array.from({length:e.length+1},(n,r)=>r);for(let n=1;n<=s.length;n++){let r=t[0];t[0]=n;for(let o=1;o<=e.length;o++){const p=t[o];t[o]=Math.min(t[o]+1,t[o-1]+1,r+(e[o-1]!==s[n-1])),r=p}}return t[e.length]}function T(e,s,t){const n=e.toLowerCase(),p=Math.max(.5,.9-(n.length>10?.15:n.length*.05));return s.reduce((d,a)=>{let $=0,x=0;return t.forEach(w=>{if(a[w]){const k=a[w].toString().toLowerCase(),y=E(n,k)/Math.max(n.length,k.length);y<=p&&($+=y,x++)}}),x>0&&d.push({item:a,score:$/x}),d},[]).sort((d,a)=>d.score-a.score)}let b="",h=[],i=[],m=[],L=[];const f=document.getElementById("search-input"),l=f?.dataset?.locale,c=f?.dataset?.t,g=f?.dataset?.scope;async function j(){const t=(await(await fetch("/site.json")).json()).data;L=Object.keys(t),L.forEach(n=>{t[n].forEach(r=>{r.type=n,m.push(r)})})}j();document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("search-input");e&&e.addEventListener("input",s=>{b=s.target.value,z()})});function z(){if(!(!m||m.length===0)){if(!b.trim()){I();return}h=T(b,m,["tv","tt","ta","id"]),i=h.reduce((e,s)=>{const t=s.item.type;return e[t]=e[t]||[],e[t].push(s),e},{}),h.length>0,M()}}function I(){const e=document.getElementById("search-results-wrapper");e.innerHTML=""}const v=(e,s)=>e.item[s]??"",u=(e,s,t,n,r="",o="")=>`
    <li class="flow-root text-center bg-stone-50 dark:bg-stone-800">
      ...
      >
        ...
        <div class="min-w-0 flex-1 ml-4 text-left">
          ...
          ${e.item.tt!==e.item.tv&&e.item.tv?`
                <p class="truncate mt-1 text-sm text-stone-500 dark:text-stone-300">
                  ${v(e,"tv")}
                </p>`:""}
        </div>
        ${o?`
  ...
  `;function M(){const e=document.getElementById("search-results-wrapper");if(e.innerHTML="",h.length===0)return;const s=`
